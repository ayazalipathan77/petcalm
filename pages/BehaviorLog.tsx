...
import { Plus, BrainCircuit, Trash2, Pencil, FileDown, Crown, Lock, Loader2 } from 'lucide-react';
...
export const BehaviorLog: React.FC<BehaviorLogProps> = ({ petName, petId }) => {
  ...
  const buildPdf = (): jsPDF => {
    ...
    incidents.forEach((incident) => {
      ...
      doc.text(incident.trigger.slice(0, 28), 55, y);
      const sev = incident.severity;
      doc.setTextColor(sev >= 4 ? 231 : sev === 3 ? 243 : 39, sev >= 4 ? 76 : sev === 3 ? 156 : 174, sev >= 4 ? 60 : sev === 3 ? 18 : 96);
      doc.text(`${sev}/5`, 115, y);
      doc.setTextColor(44, 62, 80);
      if (incident.notes) {
        const lines = doc.splitTextToSize(incident.notes, 55);
        doc.text(lines[0], 135, y);
      }
      y += 8;
    });
  ...
  const exportVetReport = async () => {
    setIsExporting(true);
    try {
      const doc = buildPdf();
      const fileName = `${petName.replace(/\s+/g, '_')}_behavior_report.pdf`;

      if (Capacitor.isNativePlatform()) {
        // On Android: write to cache directory then open system share sheet
      ...
      <div className="flex gap-3 mb-6">
        ...
        >
          {isPro
            ...
            : <><Crown size={18} className="text-amber-500" /> Export PDF (Pro)</>
          }
        </Button>
      </div>
