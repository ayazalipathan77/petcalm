...
export const Training: React.FC<TrainingProps> = ({ programs, onUpdateProgram }) => {
  ...
  const handleCompleteStep = (programId: string, stepIndex: number) => {
    ...
    const newIndex = stepIndex === program.completedStepIndex ? program.completedStepIndex + 1 : program.completedStepIndex;

    if (newIndex !== program.completedStepIndex) {
      const updatedProgram = { ...program, completedStepIndex: newIndex };
  ...
  if (selectedProgram) {
    // Ensure we are displaying the latest state of the selected program
    // We find it in the props array, or fallback to the one in local state if not found (shouldn't happen)
    const activeProgram = programs.find(p => p.id === selectedProgram.id) || selectedProgram;
    const progress = (activeProgram.completedStepIndex / activeProgram.steps.length) * 100;
      ...
      <div className="bg-neutral-bg min-h-full pb-24">
        ...
        <div className="bg-white px-6 py-6 border-b border-neutral-100 sticky top-0 z-30 shadow-sm">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-neutral-text leading-tight mb-2">{activeProgram.title}</h1>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold bg-primary/10 text-primary px-2 py-1 rounded">
                  {activeProgram.category}
                </span>
                <span className="text-xs text-neutral-400">
                  {activeProgram.steps.length} Steps
                </span>
              </div>
            </div>
            <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center text-primary flex-shrink-0">
              {getIcon(activeProgram.icon)}
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="relative space-y-8 pl-4">
            ...
            {activeProgram.steps.map((step, index) => {
              const isLocked = index > activeProgram.completedStepIndex;
              const isCompleted = index < activeProgram.completedStepIndex;
              const isCurrent = index === activeProgram.completedStepIndex;
                  ...
                  <div className={`flex-1 p-4 rounded-xl border ${isCurrent ? 'bg-white border-primary shadow-md ring-1 ring-primary/10' : 'bg-white border-neutral-200'}`}>
                    ...
                    {isCurrent && (
                      <Button
                        size="sm"
                        fullWidth
                        onClick={() => handleCompleteStep(activeProgram.id, index)}
